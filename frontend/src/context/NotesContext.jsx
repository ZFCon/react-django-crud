...
const NotesProvider = ({ children }) => {
    ...
    const addNote = (text) => {
        axios.post('http://127.0.0.1:8000/api/notes/', { "text": text }).then(response => setNotes([response.data, ...notes])).catch(error => console.error(error));
    };
    ...
    const updateNote = (id, text) => {
        axios
            .put(`http://127.0.0.1:8000/api/notes/${id}/`, { text: text })
            .then((response) =>
                setNotes(
                    notes.map((note) =>
    ...
    const getNote = (id) => {
        return axios.get(`http://127.0.0.1:8000/api/notes/${id}/`)
            .then((response) => {
                // eslint-disable-next-line
